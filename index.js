...
import { eventSource, event_types, generateRaw, name1, name2, characters } from '../../../script.js';
...
const DEFAULTS = {
    ...
    autoRoster: true,
    maxContext: 40,
    replySystem: '你正在模拟微信聊天。严格按照指定角色发送消息，不要替用户说话，不要添加旁白。一次可以返回多条消息。',
};
...
function addMessage(session, sender, receiver, text, type='text', extra={}) {
    ...
    const m={id:uid(), sender, receiver, text:String(text??''), type, time:Date.now(), countInMain:!!extra.countInMain, sourceId:extra.sourceId||null, extra};
    s.messages.push(m);
...
function sendComposed() {
    ...
    const s=ensureSession(state.me,state.target,false);
    addMessage(s.id,state.me,state.target,text,'text',{countInMain:count, sourceId:'manual:'+uid()});
...
function handleSpecial(type) {
    ...
    if(type==='image'){
        ...
        if(url){const s=ensureActiveSession();addMessage(s.id,state.me,state.target,url,'image',{countInMain:!!document.getElementById('wxsim_count_memory')?.checked});renderMessages();}
    } else if(type==='location'){
        const label=prompt('位置名称：','当前位置'); if(label){const detail=prompt('位置描述：','附近');const s=ensureActiveSession();addMessage(s.id,state.me,state.target,detail||'', 'location',{label,countInMain:!!document.getElementById('wxsim_count_memory')?.checked});renderMessages();}
    } else if(type==='redpacket'){
        const amount=prompt('红包金额：','8.88'); if(amount){const s=ensureActiveSession();addMessage(s.id,state.me,state.target,`¥${amount} · 恭喜发财，大吉大利`,'redpacket',{countInMain:!!document.getElementById('wxsim_count_memory')?.checked});renderMessages();}
    } else if(type==='transfer'){
        const amount=prompt('转账金额：','10.00'); if(amount){const s=ensureActiveSession();addMessage(s.id,state.me,state.target,amount,'transfer',{countInMain:!!document.getElementById('wxsim_count_memory')?.checked});renderMessages();}
    } else if(type==='sticker'){
        const x=prompt('表情包文字/描述：',STICKERS[Math.floor(Math.random()*STICKERS.length)]); if(x){const s=ensureActiveSession();addMessage(s.id,state.me,state.target,x,'sticker',{countInMain:!!document.getElementById('wxsim_count_memory')?.checked});renderMessages();}
    }
}
...
async function callAI(prompt) {
    ...
    if(s.apiMode==='main'){
        ...
        const out=await c.generateRaw({prompt,systemPrompt:s.replySystem||'你正在模拟微信聊天，只输出角色消息。',responseLength:1000,trimNames:false,temperature:Number(s.temperature)||0.8});
...
function bindSettings() {
    ...
    const fields={
      ...
      wxsim_model:'model',wxsim_temperature:'temperature',wxsim_batch:'batch',wxsim_max_context:'maxContext',wxsim_parse_mode:'parseMode',
      wxsim_regex:'regex',wxsim_inject_memory:'injectMemory',wxsim_auto_roster:'autoRoster',wxsim_reply_system:'replySystem'
    };
...
function init() {
    ...
    bindSettings();
    const c=ctx();
    const es=eventSource; const et=event_types;
    if(es&&et){
        ...
        if(et.CHAT_COMPLETION_PROMPT_READY) es.on(et.CHAT_COMPLETION_PROMPT_READY,()=>injectMemoryIfNeeded());
    }
    setTimeout(()=>bindSettings(),500);
    console.log('[微信模拟器] initialized v0.2.0');
}
export { init };
